...
- shaping tables are preserved in data forms needed by `he_text_layout`.
...
Runtime loaders in `he_text_vector_assets` should:

- load resource headers,
- validate versions,
- create CPU-side descriptors for render resources,
- hand immutable payload views to `he_text_vector` and `he_text_layout`.
...
invalidation metadata yet, so `he.text_vector` should explicitly plan for:

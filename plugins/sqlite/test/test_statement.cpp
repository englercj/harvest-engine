...
HE_TEST(sqlite, statement, test)
{
...

    {
        ...
        HE_EXPECT_EQ(s.Step(), StepResult::Row);
        HE_EXPECT_EQ(s.GetColumn(0).AsInt(), 50);
        HE_EXPECT_EQ(s.GetColumn(1).AsBlob().Size(), 1);
        HE_EXPECT_EQ(*s.GetColumn(1).AsBlob().Data(), 0xCD);
        HE_EXPECT_EQ(s.Step(), StepResult::Row);
        HE_EXPECT_EQ(s.GetColumn(0).AsInt(), 50);
        HE_EXPECT_EQ(s.GetColumn(1).AsBlob().Size(), 1);
        HE_EXPECT_EQ(*s.GetColumn(1).AsBlob().Data(), 0xEF);
        HE_EXPECT_EQ(s.Step(), StepResult::Done);
        HE_EXPECT(s.Reset());
        ...
        {
            ++rowCount;
            HE_EXPECT_EQ(st.GetColumn(0).AsInt(), 50);
            HE_EXPECT_EQ(s.GetColumn(1).AsBlob().Size(), 1);
        });

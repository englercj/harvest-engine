...
DOTNET_CLI_HOME="$BUILD_DIR/.dotnet"
...
# Keep dotnet first-run state local to the project so source runs work in
# restricted environments and do not depend on user-profile writes.
...
if [[ $NEEDS_REBUILD -eq 1 ]]; then
    ...
    "$DOTNET_EXE" build "$HEMAKE_SRC_PROJ" -c "$HEMAKE_BUILD_CFG" -v "$MSBUILD_VERBOSITY"

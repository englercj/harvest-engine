...
namespace he::wyhash
{
    ...
    [[nodiscard]] HE_FORCE_INLINE uint64_t Mix(uint64_t a, uint64_t b)
    {
        Mum(a, b);
        return a ^ b;
    }

    [[nodiscard]] HE_FORCE_INLINE uint64_t R8(const uint8_t* p)
    {
        uint64_t v;
        MemCopy(&v, p, 8);
        return v;
    ...
    [[nodiscard]] HE_FORCE_INLINE uint64_t R4(const uint8_t* p)
    {
        uint32_t v;
        MemCopy(&v, p, 4);
        return v;

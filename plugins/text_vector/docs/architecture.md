...
Build a new first-party plugin, `he.text_vector`, that provides:
...
3. `he_text_layout`
   ...
   - Depends on `he_core`, `he_math`, `he_text_vector_assets`, and HarfBuzz.
...
4. `he_text_vector`
   - Owns GPU resources, draw packet encoding, shader bindings, render passes, and vector/glyph
     submission APIs.
   - Depends on `he_core`, `he_math`, `he_rhi`, `he_text_vector_assets`, and
     `he_text_layout`.
...
At runtime, `he_text_vector` should load these resources into immutable GPU objects and
...
7. Submission to `he_text_vector`.

The UI library should depend on `he_text_layout` for layout and on `he_text_vector` for
rendering.
...
Keep import and compile work in `he_text_vector_editor`, but use the existing shared Harvest
asset pipeline:

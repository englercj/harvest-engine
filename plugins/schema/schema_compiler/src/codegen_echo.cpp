...
namespace he::schema
{
    ...
    bool CodeGenEcho::Generate()
    {
        ...
        for (Attribute::Reader attr : m_request.schema.Attributes())
        {
    ...
    void CodeGenEcho::WriteInterfaceDecl(Declaration::Reader decl, Declaration::Reader scope)
    {
        ...
        {
            if (!child.Data().IsStruct() || (!child.Data().Struct().IsAutoGenerated()))
            {
    ...
    void CodeGenEcho::WriteStructDecl(Declaration::Reader decl, Declaration::Reader scope)
    {
        ...
        for (Declaration::Reader child : decl.Children())
        {
            ...
            if (!childData.IsStruct() || (!childData.Struct().IsAutoGenerated() && !childData.Struct().IsGroup() && !childData.Struct().IsUnion()))
            {
                WriteDecl(child, decl);
            }
        }
        ...
        for (Field::Reader field : structDecl.Fields())
        {
            ...
            Field::Meta::Normal::Reader fieldNorm = field.Meta().Normal();
            ...
            else
            {
                ...
                const uint32_t begin = fieldNorm.DataOffset() * fieldSize;
    ...
    void CodeGenEcho::WriteField(Field::Reader field, Declaration::Reader scope)
    {
        ...
        m_writer.Write("{} @{} :", field.Name().AsView(), fieldNorm.Ordinal());
        WriteType(field.Type(), scope);

        if (fieldNorm.HasDefaultValue() && !fieldNorm.DefaultValue().Data().IsVoid())
        {
            m_writer.Write(" = ");
            WriteValue(field.Type(), scope, fieldNorm.DefaultValue());
        }

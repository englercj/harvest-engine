...
namespace he
{
    ...
    constexpr FNV32::ValueType FNV32::HashData(const void* data, size_t len, ValueType h)
    {
    ...
    template <>
    constexpr FNV32::ValueType FNV32::HashScalar(const bool& b, ValueType h)
    {
    ...
    constexpr FNV32::ValueType FNV32::HashScalar(const T& obj, ValueType h)
    {
    ...
    inline FNV32& FNV32::Data(const void* data, size_t len)
    {
    ...
    constexpr FNV64::ValueType FNV64::HashData(const void* data, size_t len, ValueType h)
    {
        const uint8_t* s = static_cast<const uint8_t*>(data);
        while (len-- > 0)
            h = _FNV64Prime * (h ^ *s++);
    ...
    template <>
    constexpr FNV64::ValueType FNV64::HashScalar(const bool& b, ValueType h)
    {
    ...
    template <typename T, HE_REQUIRED(IsArithmetic<T>)>
    constexpr FNV64::ValueType FNV64::HashScalar(const T& obj, ValueType h)
    {
        const uint8_t* s = reinterpret_cast<const uint8_t*>(&obj);
        for (size_t i = 0; i < sizeof(T); ++i)
            h = _FNV64Prime * (h ^ *s++);
    ...
    inline FNV64& FNV64::Data(const void* data, size_t len)
    {
        m_state = HashData(data, len, m_state);
        return *this;
    }

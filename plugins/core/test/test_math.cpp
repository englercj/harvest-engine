...
{
    HE_EXPECT_EQ(MathConstants<T>::E, std::numbers::e_v<T>);
    HE_EXPECT_EQ(MathConstants<T>::Log2E, std::numbers::log2e_v<T>);
    HE_EXPECT_EQ(MathConstants<T>::Log10E, std::numbers::log10e_v<T>);
    HE_EXPECT_EQ(MathConstants<T>::LogE2, std::numbers::ln2_v<T>);
    HE_EXPECT_EQ(MathConstants<T>::LogE10, std::numbers::ln10_v<T>);
    HE_EXPECT_EQ(MathConstants<T>::Pi, std::numbers::pi_v<T>);
    HE_EXPECT_EQ(MathConstants<T>::Pi2, std::numbers::pi_v<T> *T(2));
    HE_EXPECT_EQ(MathConstants<T>::PiHalf, std::numbers::pi_v<T> / T(2));
    HE_EXPECT_EQ(MathConstants<T>::PiQuarter, std::numbers::pi_v<T> / T(4));
    HE_EXPECT_EQ(MathConstants<T>::Sqrt2, std::numbers::sqrt2_v<T>);
    HE_EXPECT_EQ(MathConstants<T>::Sqrt2Rcp, T(1) / std::numbers::sqrt2_v<T>);
}

template <typename T>
static void TestConstantsEx()
{
    HE_EXPECT_EQ(Cos(MathConstants<T>::Pi), T(-1));
    HE_EXPECT_EQ(Acos(T(-1)), MathConstants<T>::Pi);

    HE_EXPECT_EQ(Cos(MathConstants<T>::Pi2), T(1));
    HE_EXPECT_EQ(MathConstants<T>::Pi2, T(2) * MathConstants<T>::Pi);

    HE_EXPECT_EQ(MathConstants<T>::PiHalf, MathConstants<T>::Pi / T(2));
    HE_EXPECT_EQ(MathConstants<T>::PiQuarter, MathConstants<T>::Pi / T(4));
    HE_EXPECT_EQ(MathConstants<T>::Sqrt2, Sqrt(T(2)));
}

HE_TEST(core, math, MathConstants)
{
    TestConstants<float>();
    TestConstants<double>();
    TestConstants<long double>();

    TestConstantsEx<float>();
    TestConstantsEx<double>();
    //TestConstantsEx<long double>();
}
...
{
    static_assert(Classify(Limits<T>::Min) == FpClass::Normal);
    static_assert(Classify(Limits<T>::MinPos) == FpClass::Normal);
    static_assert(Classify(Limits<T>::Max) == FpClass::Normal);
    static_assert(Classify(Limits<T>::Epsilon) == FpClass::Normal);
    static_assert(Classify(Limits<T>::ZeroSafe) == FpClass::Normal);
    static_assert(Classify(Limits<T>::DenormMin) == FpClass::Subnormal);
    static_assert(Classify(T(0)) == FpClass::Zero);
    static_assert(Classify(T(-0)) == FpClass::Zero);
    static_assert(Classify(Limits<T>::Infinity) == FpClass::Infinite);
    static_assert(Classify(Limits<T>::NaN) == FpClass::Nan);
    static_assert(Classify(Limits<T>::SignalingNaN) == FpClass::Nan);

    HE_EXPECT_EQ(Classify(Limits<T>::Min), FpClass::Normal);
    HE_EXPECT_EQ(Classify(Limits<T>::MinPos), FpClass::Normal);
    HE_EXPECT_EQ(Classify(Limits<T>::Max), FpClass::Normal);
    HE_EXPECT_EQ(Classify(Limits<T>::Epsilon), FpClass::Normal);
    HE_EXPECT_EQ(Classify(Limits<T>::ZeroSafe), FpClass::Normal);
    HE_EXPECT_EQ(Classify(Limits<T>::DenormMin), FpClass::Subnormal);
    HE_EXPECT_EQ(Classify(T(0)), FpClass::Zero);
    HE_EXPECT_EQ(Classify(T(-0)), FpClass::Zero);
    HE_EXPECT_EQ(Classify(Limits<T>::Infinity), FpClass::Infinite);
    HE_EXPECT_EQ(Classify(Limits<T>::NaN), FpClass::Nan);
    HE_EXPECT_EQ(Classify(Limits<T>::SignalingNaN), FpClass::Nan);
}

HE_TEST(core, math, Classify)
{
    TestClassify<float>();
    TestClassify<double>();
    TestClassify<long double>();
}

// ------------------------------------------------------------------------------------------------
HE_TEST(core, math, IsNan)
{
...
HE_TEST(core, math, IsInfinite)
{
    ...
    static_assert(!IsInfinite(Limits<float>::NaN));
    static_assert(!IsInfinite(Limits<float>::SignalingNaN));
    ...
    HE_EXPECT(!IsInfinite(Limits<float>::NaN));
    HE_EXPECT(!IsInfinite(Limits<float>::SignalingNaN));
}

// ------------------------------------------------------------------------------------------------
HE_TEST(core, math, IsFinite)
{
    ...
    static_assert(!IsFinite(Limits<float>::NaN));
    static_assert(!IsFinite(Limits<float>::SignalingNaN));
    ...
    HE_EXPECT(!IsFinite(Limits<float>::NaN));
    HE_EXPECT(!IsFinite(Limits<float>::SignalingNaN));
}

// ------------------------------------------------------------------------------------------------
HE_TEST(core, math, IsNormal)
{
    static_assert(IsNormal(Limits<float>::Min));
    static_assert(IsNormal(Limits<float>::Max));
    static_assert(IsNormal(Limits<float>::Epsilon));
    static_assert(!IsNormal(Limits<float>::Infinity));
    static_assert(!IsNormal(Limits<float>::NaN));
    static_assert(!IsNormal(Limits<float>::SignalingNaN));

    HE_EXPECT(IsNormal(Limits<float>::Min));
    HE_EXPECT(IsNormal(Limits<float>::Max));
    HE_EXPECT(IsNormal(Limits<float>::Epsilon));
    HE_EXPECT(!IsNormal(Limits<float>::Infinity));
    HE_EXPECT(!IsNormal(Limits<float>::NaN));
    HE_EXPECT(!IsNormal(Limits<float>::SignalingNaN));
}

// ------------------------------------------------------------------------------------------------
HE_TEST(core, math, IsZeroSafe)
{
...
// ------------------------------------------------------------------------------------------------
HE_TEST(core, math, IsNearlyEqual)
{
    // TODO: constexpr
    HE_EXPECT(IsNearlyEqual(-0.0f, 0.0f, 0.0f));
    HE_EXPECT(IsNearlyEqual(0.0f, -0.0f, 0.0f));
    HE_EXPECT(IsNearlyEqual(0.0f, 1.0f, 1.0f));
    HE_EXPECT(IsNearlyEqual(1.0f, 0.0f, 1.0f));
}

// ------------------------------------------------------------------------------------------------
HE_TEST(core, math, IsNearlyEqualULP)
{
    // TODO: constexpr
    HE_EXPECT(IsNearlyEqualULP(-0.0f, 0.0f, 0));
    HE_EXPECT(IsNearlyEqualULP(0.0f, -0.0f, 0));
    HE_EXPECT(IsNearlyEqualULP(0.0f, 1.0f, 0x3f800000));
    HE_EXPECT(IsNearlyEqualULP(1.0f, 0.0f, 0x3f800000));
}
...
// ------------------------------------------------------------------------------------------------
HE_TEST(core, math, Rsqrt)
{
    HE_EXPECT_EQ(Rsqrt(4.0f), 0.5f);
}

// ------------------------------------------------------------------------------------------------
HE_TEST(core, math, Sin)
{
    ...
    HE_EXPECT_EQ(Sin(0.0f), std::sinf(0));
    HE_EXPECT_EQ(Sin(MathConstants<float>::PiQuarter), std::sinf(MathConstants<float>::PiQuarter));
    HE_EXPECT_EQ(Sin(MathConstants<float>::PiHalf), std::sinf(MathConstants<float>::PiHalf));
    HE_EXPECT_EQ(Sin(MathConstants<float>::Pi), std::sinf(MathConstants<float>::Pi));
    HE_EXPECT_EQ(Sin(MathConstants<float>::Pi2), std::sinf(MathConstants<float>::Pi2));
}

// ------------------------------------------------------------------------------------------------
HE_TEST(core, math, Asin)
{
    ...
    // sampling of some test values against cmath implementations
    HE_EXPECT_EQ(Asin(0.0f), std::asinf(0.0f));
    HE_EXPECT_EQ(Asin(0.5f), std::asinf(0.5f));
    HE_EXPECT_EQ(Asin(1.0f), std::asinf(1.0f));
    HE_EXPECT_EQ(Asin(-1.0f), std::asinf(-1.0f));
    HE_EXPECT_EQ(Asin(-0.5f), std::asinf(-0.5f));
}

// ------------------------------------------------------------------------------------------------
HE_TEST(core, math, Cos)
{
    ...
    // sampling of some test values against cmath implementations
    HE_EXPECT_EQ(Cos(0.0f), std::cosf(0.0f));
    HE_EXPECT_EQ(Cos(MathConstants<float>::PiQuarter), std::cosf(MathConstants<float>::PiQuarter));
    HE_EXPECT_EQ(Cos(MathConstants<float>::PiHalf), std::cosf(MathConstants<float>::PiHalf));
    HE_EXPECT_EQ(Cos(MathConstants<float>::Pi), std::cosf(MathConstants<float>::Pi));
    HE_EXPECT_EQ(Cos(MathConstants<float>::Pi2), std::cosf(MathConstants<float>::Pi2));
}

// ------------------------------------------------------------------------------------------------
HE_TEST(core, math, Acos)
{
    ...
    // sampling of some test values against cmath implementations
    HE_EXPECT_EQ(Acos(0.0f), std::acosf(0.0f));
    HE_EXPECT_EQ(Acos(0.5f), std::acosf(0.5f));
    HE_EXPECT_EQ(Acos(1.0f), std::acosf(1.0f));
    HE_EXPECT_EQ(Acos(-1.0f), std::acosf(-1.0f));
    HE_EXPECT_EQ(Acos(-0.5f), std::acosf(-0.5f));
}

// ------------------------------------------------------------------------------------------------
HE_TEST(core, math, Tan)
{
    ...
    // sampling of some test values against cmath implementations
    HE_EXPECT_EQ(Tan(0.0f), std::tanf(0.0f));
    HE_EXPECT_EQ(Tan(MathConstants<float>::PiQuarter), std::tanf(MathConstants<float>::PiQuarter));
#if !HE_COMPILER_GCC
    // TODO: Somehow this test fails on GCC. Need to look at the generated code to know why...
    HE_EXPECT_EQ(Tan(MathConstants<float>::PiHalf), std::tanf(MathConstants<float>::PiHalf));
#endif
    HE_EXPECT_EQ(Tan(MathConstants<float>::Pi), std::tanf(MathConstants<float>::Pi));
    HE_EXPECT_EQ(Tan(MathConstants<float>::Pi2), std::tanf(MathConstants<float>::Pi2));
}

// ------------------------------------------------------------------------------------------------
HE_TEST(core, math, Atan)
{
    ...
    // sampling of some test values against cmath implementations
    HE_EXPECT_EQ(Atan(0.0f), std::atanf(0.0f));
    HE_EXPECT_EQ(Atan(MathConstants<float>::PiQuarter), std::atanf(MathConstants<float>::PiQuarter));
    HE_EXPECT_EQ(Atan(MathConstants<float>::PiHalf), std::atanf(MathConstants<float>::PiHalf));
    HE_EXPECT_EQ(Atan(MathConstants<float>::Pi), std::atanf(MathConstants<float>::Pi));
    HE_EXPECT_EQ(Atan(MathConstants<float>::Pi2), std::atanf(MathConstants<float>::Pi2));
}

// ------------------------------------------------------------------------------------------------
HE_TEST(core, math, Atan2)
{
...
// ------------------------------------------------------------------------------------------------
HE_TEST(core, math, SinCos)
{
    // TODO
}

// ------------------------------------------------------------------------------------------------
HE_TEST(core, math, Exp)
{
...
// ------------------------------------------------------------------------------------------------
HE_TEST(core, math, Log)
{
    // https://en.cppreference.com/w/cpp/numeric/math/log

    // If the argument is 1, +0 is returned
    HE_EXPECT_EQ(Log(1.0f), 0.0f);

    // If the argument is +∞, +∞ is returned
    HE_EXPECT_EQ(Log(Limits<float>::Infinity), Limits<float>::Infinity);
}

// ------------------------------------------------------------------------------------------------
HE_TEST(core, math, Log2)
{
    // https://en.cppreference.com/w/cpp/numeric/math/log2

    // If the argument is 1, +0 is returned
    HE_EXPECT_EQ(Log2(1.0f), 0.0f);

    // If the argument is +∞, +∞ is returned
    HE_EXPECT_EQ(Log2(Limits<float>::Infinity), Limits<float>::Infinity);

    // sampling of some known test values
    HE_EXPECT_EQ(Log2(65536.0f), 16.0f);
    HE_EXPECT_EQ(Log2(0.125f), -3.0f);
    HE_EXPECT_EQ(Log2(static_cast<float>(0x020f)), 9.04165936f);
    HE_EXPECT_EQ(Log2(1.0f), 0.0f);
}

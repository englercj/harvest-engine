...
internal class ProjectService : IProjectService
{
    ...
    private void ResolveImports(List<KdlNode> importNodes)
    {
        foreach (KdlNode importNode in importNodes)
        {
            ...
            List<KdlNode> resolvedImportedNodes = [];
            List<string> importFileMatches = [];

            List<string> searchPaths = [importPath];
            if (Path.GetExtension(importPath) != ".kdl")
            {
                searchPaths.Add(importPath + ".kdl");
                searchPaths.Add(Path.Combine(importPath, "he_plugin.kdl"));
            }

            foreach (string searchPath in searchPaths)
            {
                IReadOnlyList<string> expandedPaths = PathGlobs.ExpandPath(searchPath, directory);
                importFileMatches =
                [
                    .. expandedPaths.Where((path) => !Directory.Exists(path))
                ];

                if (importFileMatches.Count > 0)
                {
                    break;
                }
            }

            foreach (string importMatchedPath in importFileMatches)
            {
                KdlDocument importedDoc = LoadFile(importMatchedPath);
                ResolveImports(importedDoc);

                foreach (KdlNode node in importedDoc.Nodes)
                {
                    resolvedImportedNodes.Add(node.Clone(includeChildren: true));
                }
            }

            if (resolvedImportedNodes.Count == 0)
            {
                throw new NodeParseException(importNode, $"Import path did not match any files: '{importPath}'.");
            }

            importNode.ReplaceInParent(resolvedImportedNodes);
        }
    }
}

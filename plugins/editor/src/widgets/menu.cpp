...
namespace he::editor
{
    ...
    static bool ImGuiIsRootOfOpenMenuSet()
    {
    ...
    static bool ImGuiBeginMenuEx(const char* label, const char* icon, bool enabled)
    {
        ...
        //////////
        // What follows is a copy-paste of the ImGui::MenuItemEx implementation with a few minor
        // modifications to support our menu styles.
        //////////
        ...
        //////////
        // HARVEST CHANGE START
        //const bool menuset_is_open = IsRootOfOpenMenuSet();
        const bool menuset_is_open = ImGuiIsRootOfOpenMenuSet();
        // HARVEST CHANGE END
        //////////
        if (menuset_is_open)
        ...
        else
        {
            ...
            float checkmark_w = style.ItemInnerSpacing.x + CalcTextSize(ICON_MDI_CHECK, NULL).x;
            // HARVEST CHANGE END
            //////////
            ...
            window->ParentWorkRect.Min.x -= 2;
            pressed = Selectable("", menu_is_open, selectable_flags | ImGuiSelectableFlags_SpanAvailWidth | ImGuiSelectableFlags_SpanAllColumns, ImVec2(min_w, label_size.y));
            window->ParentWorkRect.Min.x += 2;
            // HARVEST CHANGE END
            //////////
            RenderText(text_pos, label);
            ...
            RenderText(pos + ImVec2((offsets->OffsetMark + extra_w + g.FontSize * 0.30f) - (style.ItemSpacing.x * 0.5f), 0.0f), ICON_MDI_CHEVRON_RIGHT, nullptr, false);
            // HARVEST CHANGE END
            //////////
        }
        if (!enabled)
    ...
    static bool ImGuiMenuItemEx(const char* label, const char* icon, const char* shortcut, bool selected, bool enabled)
    {
        ...
        else
        {
            ...
            window->ParentWorkRect.Min.x -= 2;
            pressed = Selectable("", false, selectable_flags | ImGuiSelectableFlags_SpanAvailWidth | ImGuiSelectableFlags_SpanAllColumns, ImVec2(min_w, label_size.y));
            window->ParentWorkRect.Min.x += 2;
            // HARVEST CHANGE END
            //////////
            if (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible)
            {
                ...
                if (shortcut_w > 0.0f)
                {
                    ...
                    RenderText(pos + ImVec2(min_w - shortcut_w - checkmark_w - style.ItemInnerSpacing.x + stretch_w, 0.0f), shortcut, nullptr, false);
                    // HARVEST CHANGE END
                    //////////
                    PopStyleColor();
                }
                ...
                if (selected)
                    RenderText(pos + ImVec2(min_w - checkmark_w + stretch_w, 0), ICON_MDI_CHECK, nullptr, false);
                // HARVEST CHANGE END
                //////////
            }
        }
    ...
    void TopLevelIcon()
    {
        ...
        ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(8.0f, 0.0f) * g.CurrentDpiScale);
    ...
    bool BeginMenu(const char* label, const char* icon, bool enabled)
    {
        ...
        return ImGuiBeginMenuEx(label, icon, enabled);
    }
    ...
    void MenuSystemButtons(window::View* view, window::ViewHitArea& hitArea)
    {
        ...
        // Minimize
        if (ImGui::Button(ICON_MDI_WINDOW_MINIMIZE "##minimize_sys_button", sysButtonSize))
            view->Minimize();
        if (ImGui::IsItemHovered())
            hitArea = window::ViewHitArea::ButtonMinimize;

        // Maximize & Restore
        constexpr char RestoreButtonText[] = ICON_MDI_WINDOW_RESTORE "##restore_sys_button";
        constexpr char MaximizeButtonText[] = ICON_MDI_WINDOW_MAXIMIZE "##maximize_sys_button";
        if (ImGui::Button(view->IsMaximized() ? RestoreButtonText : MaximizeButtonText, sysButtonSize))
            view->ToggleMaximize();
        if (ImGui::IsItemHovered())
        ...
        ImGui::PushStyleColor(ImGuiCol_ButtonActive, style.Colors[ImGuiCol_ButtonHovered]);
        if (ImGui::Button(ICON_MDI_CLOSE "##close_sys_button", sysButtonSize))
            view->RequestClose();
        if (ImGui::IsItemHovered())
